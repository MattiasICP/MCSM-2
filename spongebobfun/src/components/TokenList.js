            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-purple-900/30">
                {filteredAndSortedTokens.map(([tokenInfo, liquidityPool], index) => {
                  ...
                  const cachedInfo = launchBobService.getCachedTokenInfo(tokenInfo.created_at);
                  ...
                  return (
                    <tr
                      key={`${tokenInfo.created_at}-${index}`}
                      className="hover:bg-purple-900/10 transition-colors cursor-pointer"
                      onClick={() => {
                        if (tokenInfo.created_at && tokenInfo.created_by) {
                          onSelectToken(tokenInfo, liquidityPool);
                        } else {
                          console.error("Token ID is undefined:", tokenInfo);
                        }
                      }}
                    >
