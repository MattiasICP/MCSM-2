...
const TokenTrading = ({ token, liquidityPool, launchBobService, onBack }) => {
  ...
  const fetchTokenData = useCallback(async () => {
    if (!liquidityPool?.token_id) {
      console.error('No token ID provided in LiquidityPool:', liquidityPool);
      setError('Invalid token - no token ID found in LiquidityPool');
      setLoading(false);
      return;
    }
    ...
    try {
      setLoading(true);
      const data = await launchBobService.getTokenData(liquidityPool.token_id);
      if (!data) {
        throw new Error('No data returned for token');
      }
      setTokenData(data);
      
      const cachedInfo = launchBobService.getCachedTokenInfo(liquidityPool.token_id);
      if (cachedInfo) {
      ...
      if (principal) {
        const balance = await launchBobService.getTokenBalance(liquidityPool.token_id, principal);
  ...
  }, [liquidityPool, launchBobService]);
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      const spenderPrincipal = launchBobService.getPrincipal();

      // Step 1: Approve the amount with the correct spender structure
      await icpLedgerService.approveSpender(spenderPrincipal, spenderPrincipal, amountE8s);

      // Step 2: Attempt the trade
      let result;
      if (tradeType === 'buy') {
        result = await launchBobService.buyToken(liquidityPool.token_id, amountE8s);
      } else {
        result = await launchBobService.sellToken(liquidityPool.token_id, amountE8s);
      }
