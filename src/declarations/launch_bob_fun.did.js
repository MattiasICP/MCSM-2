export const idlFactory = ({ IDL }) => {
  ...
  const CreatePosition = IDL.Record({
    'tickUpper': IDL.Int32,
  ...
  return IDL.Service({
    ...
    'get_pending_transfers': IDL.Func([], [IDL.Vec(PendingTransfer)], ['query']),
    ...
    'get_token_balances': IDL.Func([IDL.Principal], [IDL.Vec(Balance)], ['query']),
...
// Optional: Add utility functions for working with the types
export const PrincipalFromText = (text) => {
  return { 'Principal': text };
};

export const NatFromNumber = (num) => {
